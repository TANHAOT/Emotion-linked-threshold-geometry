...
def main():
    ...
    p.add_argument('--start', type=int, default=0); p.add_argument('--end', type=int, default=1017)
    p.add_argument('--output-dir', type=Path, default=Path(__file__).resolve().parent/'inputs')
    p.add_argument('--data-dir', type=Path, default=ROOT/'data')
    a=p.parse_args(); demographics,_=load_human_data(a.data_dir)
    if not 0 <= a.start < a.end <= len(demographics): p.error(f'valid range: 0..{len(demographics)}')
