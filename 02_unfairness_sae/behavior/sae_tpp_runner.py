...
SECTION_DIR = CURRENT_DIR.parent
...
parser.add_argument("--sae-config", "--sae_config", dest="sae_config", type=str, default=str(SECTION_DIR / "config" / "qwen3_32b_reference.json"), help="JSON profile describing the base model, SAE checkpoint, hook, and generation defaults.")
...
INPUT_DIR = str(Path(args.input_dir or (SECTION_DIR / "inputs")).expanduser().resolve())
OUTPUT_ROOT = str(Path(args.output_root or (SECTION_DIR / "results")).expanduser().resolve())

profile = apply_sae_overrides(load_sae_profile(args.sae_config), args)
print("SAE_CONFIG", describe_profile(profile))
runtime = load_sae_runtime(profile, SECTION_DIR / "vendor")
tokenizer, model, sae = runtime.tokenizer, runtime.model, runtime.sae
model_utils_path = str((SECTION_DIR / "vendor").resolve())
