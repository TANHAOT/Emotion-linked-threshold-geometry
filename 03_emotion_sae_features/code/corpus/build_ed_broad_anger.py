#!/usr/bin/env python3
"""Build a balanced, EmpatheticDialogues-only broad anger corpus."""
...
POSITIVE_LABELS = ("angry", "furious", "annoyed")
CONTROL_LABELS = (
    "disgusted", "afraid", "sad", "disappointed", "guilty", "caring", "content"
)
EXPLICIT = re.compile(
    r"\b(anger|angry|furious|fury|rage|enraged|mad|irate|outraged|annoyed|annoyance)\b",
    re.I,
)
...
def build(args):
    ...
    for split in ("discovery", "heldout"):
        ...
        for label, selected in ((1, positives), (0, controls)):
            for row in selected:
                item = dict(row)
                item["labels"] = {"anger": label}
                item["anger_kind"] = (
                    "explicit" if label == 1 and EXPLICIT.search(item["text"]) else "implicit"
                )
                item["anger_subtype"] = item["source_label"] if label == 1 else "control"
                output.append(item)
        counts[split] = {
